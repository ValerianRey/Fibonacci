...
class INQScheduler(object):
    ...
    def quantize(self):
        ...
        for group in self.optimizer.param_groups:
            ...
            for idx, p in enumerate(group['params']):
                ...
                # quantizer = partial(self.quantize_weight_fib)
    ...
    def quantize_int(self):
        ...
        for group in self.optimizer.param_groups:
            ...
            for idx, p in enumerate(group['params']):
                ...
                T = group['Ts'][idx]
                device = p.data.device
                p.data = scale_tensor(p.data.clone().cpu(), bits=8).to(device)
    ...
    def step(self):
        ...
        """
        print("Increasing the proportion of quantized weights to {}".format(self.iterative_steps[self.idx]))

        for group in self.optimizer.param_groups:
            if group['weight_bits'] is None:
                # print("None weight bits" + repr(group))
                continue
            for idx, p in enumerate(group['params']):
                if p.requires_grad is False:
                    continue
...
def reset_lr_scheduler(scheduler):
    ...
    # if we uncomment this we get a scheduler step before the optimizer step which is not what we want
